...
// Modified query for admin to include search
if ($role == 'admin') {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $search_query = trim($_GET['search']);

        // Prepare search query with multiple columns
        ...
        // If no search is performed, show all records
        $result = $conn->query("SELECT * FROM mahasiswa");
    }
} else {
    // Keep existing query for non-admin users
    ...
    <div class="container mx-auto mt-5 px-4">
        ...
        <?php if ($role == 'admin') { ?>
            ...
            <form method="POST" action="" class="bg-white p-4 rounded shadow-md mb-4">
                <?php if ($edit_mode) { ?>
                    <h3 class="text-lg font-semibold mb-3">Edit Player Data</h3>
                    <input type="hidden" name="id" value="<?= $edit_data['id'] ?>">
                <?php } else { ?>
                    <h3 class="text-lg font-semibold mb-3">Add Player Data</h3>
                <?php } ?>
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left">Status ID</th>
                        <th class="px-4 py-2 text-left">Dorm</th>
                        <?php if ($role == 'admin') { ?>
